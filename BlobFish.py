...
if args.allvsall:
	...
	parser.add_argument('--paths', type=str,required=True,  help="path to salmon output directories (multiple directories are separated by \" \", eachdirecotry contain a quant.sf file", nargs='+')
        parser.add_argument('--conditions', type=str,required=True,  help="Condition linked to each sample, separated by \" \", a condition must be given for each input path", nargs='+')
	...
	if not len(args.paths) == len(args.conditions):
		print "Error: you need to specify one condition for each input path"
		quit()
	...
	for i in range(0,len(args.paths)):
		if not args.conditions[i] in sample_list:
			sample_list[args.conditions[i]] = []

		sample_list[args.conditions[i]].append(os.path.join(args.paths[i], 'quant.sf'))
